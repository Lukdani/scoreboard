...
export default class GameController {
  constructor() {
    ...
    this.playerNameInputElement = document.querySelector(
      '#playerNameElementInput',
    );
    this.holesInputElement = document.querySelector(
      '#holesInputElement',
    );
  ...
  setupEventListeners = () => {
    ...
    this.resetButton.addEventListener('click', () => {
      this.resetGame(this.playerName, this.numberOfHoles, null);
    });
  ...
  toggleResetButtons = () => {
    ...
    if (!this.gameDataInput.classList.contains('hideElement')) {
      ...
      if (savedScoreBoard) {
        const parsedScoreBoard = jsonTryParse(savedScoreBoard);
        this.restoreButton.title = `Restored at: ${new Date(
          parsedScoreBoard.restoreTime,
        ...
        this.restoreButton.innerHTML = `Restore from cache  😱`;
  ...
  startGame(playerName, numberOfHoles, currentScore) {
    ...
    this.playerNameElement.innerHTML = `${playerName || 'Uknown'}'${
      playerName[playerName.length - 1]?.toLowerCase() !== 's'
        ? 's'
        : ''
    } scorecard`;
  }
  ...
  restoreGame() {
    ...
    if (stringifiedSavedGame != null) {
      ...
      if (parsedGame)
        this.resetGame(
          parsedGame.playerName,
          parsedGame.numberOfHoles,
          parsedGame.currentScore,
        );
    }
  }
}
